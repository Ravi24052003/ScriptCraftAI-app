...
import { completeTheParaAsync, correctSpellingAndGrammarAsync, setStatus, updateScriptAsync } from './scriptSlice';
import { useNavigate } from 'react-router-dom';
import MainHeader from '../../components/header/MainHeader';

const CreateScript = () => {
    const editScript = useSelector(state=> state.script.script);
    const [scriptTitle, setScriptTitle] = useState(editScript?.scriptTitle || "");
    const [genre, setGenre] = useState(editScript?.genre || "");
    const [script, setScript] = useState(editScript?.script || "");
    ...
    const handleSubmit = function(e){
      ...
      dispatch(updateScriptAsync({scriptTitle, genre, script, id: editScript._id}))
  ...
  return (
    <div>
      <MainHeader />
      <h1 className=' font-bold text-center text-2xl mb-4'>Edit Script</h1>
...
<label htmlFor="script">Script</label>
        ...
        <p className=' text-red-500 w-[250px] text-sm font-semibold md:w-[90%]'>{(editScript?._id == status?.scriptId) && status?.scriptError}</p>
</div>
...
<p className=' text-red-500 w-[250px] text-sm font-semibold'>{completeTheParaStatus?.completeParaError }</p>
        ...
        <button className=' bg-green-500 px-4 py-1 rounded text-white mb-4'>Save</button>
</div>
